...
class ThreadTestCase(TestCase):
    ...
    def test_add_users_to_thread(self):
        ...
        self.assertEquals(len(self.thread.users.all()),2 )

    def test_filter_thread_by_users(self):
        self.thread.users.add(self.user1, self.user2)
        threads = Thread.objects.filter(users=self.user1).filter(users=self.user2)
        self.assertEquals(self.thread, threads[0])

    def test_filter_non_existent_thread(self):
        threads = Thread.objects.filter(users=self.user1).filter(users=self.user2)
        self.assertEquals(len(threads),0)

    def test_add_messages_to_thread(self):
        self.thread.users.add(self.user1, self.user2)     
        message1 = Message.objects.create(user=self.user1, content="Muy Buenas")   
        message2 = Message.objects.create(user=self.user2, content="Hola buenos dias")   
    ...
    def test_add_message_from_user_not_in_thread(self):
        ...
        message1 = Message.objects.create(user=self.user1, content="Muy Buenas")   
        message2 = Message.objects.create(user=self.user2, content="Hola buenos dias")
        message3 = Message.objects.create(user=self.user3, content="Hola soy un espia")   
        self.thread.messages.add(message1,message2,message3)
        self.assertEquals(len(self.thread.messages.all()),2)

    def test_find_thread_with_custom_maneger(self):
        self.thread.users.add(self.user1, self.user2)    
        thread = Thread.objects.find(self.user1,self.user2)
        self.assertEquals(self.thread, thread)

    def test_find_or_create_thread_with_custom_maneger(self):
        self.thread.users.add(self.user1, self.user2)    
